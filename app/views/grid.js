...
export default Ember.View.extend({
    ...
    init: function () {
        ...
        Ember.$(window).on('resize', _.bind(this.screenResize, this));
    },

    willDestroy: function () {
        Ember.$(window).off('resize', _.bind(this.screenResize, this));
    },
    ...
    computeNbColumns: function() {
        ...
        while ((Math.round(Ember.$(window).width() / nbColumns) < this.settings.minColumnWidth) && (nbColumns > 1)) {
            nbColumns--;
        }
        return nbColumns;
    },
    ...
    computeColumns: function () {
        ...
        // columns definition
        var columnClass = "column-s" + String(Math.round(12 / this.nbColumns));
        for (i = 1; i <= this.nbColumns; i++) {
            cols.push({
                'id': i,
                'class': columnClass,
                'items': []
